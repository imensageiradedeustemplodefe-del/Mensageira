...
export const GET = handler(async () => {
  await requireAdmin();
  const [testimonies, prayers, events, streams, photos, albums, contacts] = await Promise.all([
    ...
    prisma.contactMessage.findMany({ select: { id: true, name: true, subject: true, isRead: true, createdAt: true } }),
  ]);
  return json(snake({ testimonies, prayers, events, streams, photos, albums, contacts }));
});
